...
Windows에서 실측(`cargo test --workspace` = **119 그린**; CI는 `windows-latest`(정본 — 수용 A/B/C/D가 실제로 실행되는 곳)와 `ubuntu-latest`(프런트엔드) 두 잡, 툴체인 핀):
...
- **현재 언어** — `f64` / `bool` / `i32`, `if` / `return`, **실패 가능 함수**(`-> T!` = 정수 status +
  out-param), **지역 변수**(`let` / `let mut` + 대입), **`while` 루프**.
