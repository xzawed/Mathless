...
> **▶ 다음 세션 — 여기서 시작한다.** 순서대로:
>
> 1. **넓은 중간값 SPEC 확인 — 사용자 결정.** 초안은 `docs/slices/SPEC-wide-intermediates.md`이고 §4의 DP-W1~W11이
>    확인을 기다린다(방향 포함). 헬퍼 체크 전파 뒤로 네 변형을 다시 쟀고, 권고는 **순수 트리만**(멈춤·앞지름 없이 참값이
>    드는데 `-3`인 행 59 → 20)이다. 시험 구현은 로컬 브랜치 `exp/wide2-local`에 있다(푸시 안 함).
