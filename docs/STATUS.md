...
- **테스트:** `cargo test --workspace` = **143 pass / 0 fail**. `clippy -D warnings` clean, `fmt` clean.
...
### 3a. 즉시 재개 (확인/블록 대기)
- **내부 함수·호출 SPEC (PR #56) — 사용자 확인 대기.** 실측으로 고른 다음 슬라이스: 지금은
  **로직을 나눌 수단이 아예 없다**(비-export `fn` 선언 불가, 함수 간 호출 불가 — 우회 불가).
  **핵심 결정은 §5.1(재귀 금지)**: `while`의 비종료는 판정 불가라 계약 문서화가 최선이었지만,
  호출 그래프 사이클은 **판정 가능**하고 결과는 더 나쁘다(정지가 아니라 **프로세스 종료**).
