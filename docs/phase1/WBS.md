...
| **W17** | **수치 변환 `as`** — 그전엔 단가 × 수량이 안 됐다 | [`SPEC-numeric-conversion.md`](../slices/SPEC-numeric-conversion.md) | `qty as f64`로 곱셈 가능(이전엔 루프로 흉내); `true as i32` 거부; 암시적 혼합은 여전히 거부; `-x as f64` = `-(x as f64)`. **경계 4종을 실제 모듈에서 측정**: 절단(3.9→3, -3.9→-3)·포화(±inf→MAX/MIN)·NaN→0·`i32::MAX as f64` 정확. C 호스트 통과 | #58(SPEC) / #59 |
