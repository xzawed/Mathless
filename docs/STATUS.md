...
> **▶ 다음 세션 — 여기서 시작한다.** 순서대로:
>
> 1. **상수 선언 구현 — 진행 중.** SPEC(`docs/slices/SPEC-constants.md`)과 DP-K1~K8을 사용자가 확인했다
>    (2026-09-26). §6의 두 PR 중 **2가 남았다**: `order.mls`를 `export const`로 다시 쓰고, C 호스트가 번호를
>    바꾼 모듈을 로드에서 거부하는 것(수용 E)과 음수·`i32::MIN`의 C·FPC 컴파일(수용 C)을 잰다.
