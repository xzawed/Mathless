...
fn build(tag: &str) -> (common::TempOut, Module) {
    ...
    let out = common::TempOut::new(&format!("sret_{tag}"));
...
fn the_module_gains_no_export_and_its_imports_are_compared_not_claimed() {
    ...
    assert_eq!(
        names,
        vec![
            "ml_iface_hash_carrier".to_string(),
    ...
    let base_out = common::TempOut::new("sret_cbase");
...
fn a_failing_string_return_leaves_the_buffer_alone_but_not_a_declared_out() {
    ...
    let out = common::TempOut::new("sret_mix");
