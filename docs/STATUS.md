...
여덟 개가 쌓여 있어 성격별로 나눠 둔다. **공개 직전에 처리할 것 = 4·5·7**(+6은 저절로 풀린다),
**언어 설계 = 1·2**(다음 슬라이스와 함께 물으면 된다), **범위 = 8**, **법률 = 3**(급하지 않다고
하셨다). 다음 세션이 §3a로 바로 갈 거라면 **1번만 먼저 물으면 된다** — 파서를 건드리는 유일한 항목이다.
