...
- **테스트:** `cargo test --workspace` = **133 pass / 0 fail**. `clippy -D warnings` clean, `fmt` clean.
...
### 3a. 즉시 재개 (확인/블록 대기)
- **논리 연산자 `&&`/`||` SPEC (PR #54) — 사용자 확인 대기.** 승인하신 "단항 → `&&`" 순서의 두 번째다.
  핵심 판단은 **DP-B2(단락 평가)**: 오늘은 관측할 수단이 없지만(호출 식 없음, `f64` 0나눗셈은 inf)
  lowering이 Rust `&&`라 **구현은 이미 단락 평가한다** → 명세하되 "측정했다"고는 쓰지 않는다.
