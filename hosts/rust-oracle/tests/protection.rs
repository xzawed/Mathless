...
fn produced_module_exports_only_intended_symbols_and_is_stripped() {
    ...
    // ...and the document that publishes it has to state the number this just measured.
    //
    // The assertion above pins the FACT; it does not pin what the documents say about it.
    // `SECURITY.md` publishes "정확히 3개" as a P0 protection proxy — it is the export set's
    // home. `STATUS.md` repeated it until the one-home pass and now points there instead. The
    // set moved 2 → 3 once already, when the fingerprint slice landed.
    //
