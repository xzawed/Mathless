...
> **권장 다음:** `i32` 나눗셈·나머지. 실제 구멍이고(`cents / 100`), 자체 위험 설계가 필요하며
> (`i32 /0`이 호출 스레드를 무한히 돈다 — 프로세스를 죽이는 것이 아니다, `STATUS.md` §5-4;
> 재귀·`while`과 갈리는 지점은 **런타임 판정 가능**), **`&&` 단락 평가 부채를 갚을
> 유력 후보**다(트랩 가능한 첫 연산). 근거는 `docs/STATUS.md` §3a.
