...
때문이다: `f64` 0나눗셈은 `inf`, 무한 함수에서 `as`는 포화하고 정수 산술은 wrap — `compiler/src/codegen.rs`가
`i32`의 `+ - *`에 `wrapping_add`/`_sub`/`_mul`을 **명시적으로 방출**하므로 프로필 설정과 무관하다
(`-i32::MIN == i32::MIN`으로 **실측**됨 — SPEC-unary, §9-55.4). `!` 본문에서는 둘 다 `checked_*`와 범위
비교로 `ML_ST_OVERFLOW`(`-3`)를 **반환**한다 — 패닉이 아니다(SPEC-checked-arithmetic). 그리고 `i32`의 `/`·`%`는
**가드를 달아** 방출된다(#76 — 제수가 0이면 0을 돌려주고, `i32::MIN / -1`은 무한 함수에서 `wrapping_div`로,
`!` 본문에서 `-3`으로 간다).
...
- **조용한 오답은 기능에 귀속하기 전에 대조한다** — 그 기능이 있어도 같은 실수가 똑같이 쉬우면 근거가 아니다. 판정은
  가드를 넣은 신중한 판이 아니라 가장 직접적인 형태로 한다. 이 둘이 1라운드 후보 다섯을 내렸다(§9-69.1).
- **심기의 결과는 `--no-fail-fast`로 읽는다.** `cargo test`는 첫 실패 바이너리에서 멈춰, 그 뒤 바이너리의 테스트는
  돌지 않은 채 *"하나만 잡았다"* 처럼 보인다(§9-70.5).
...
> ## ▶ 여기서 시작한다
>
> ### 2026-09-26 기준 (6) — **헬퍼 체크 전파 슬라이스가 닫혔다: `!`를 위해 실행되는 산술은 어디서든 검사된다 (§9-72)**
>
> §9-71의 권고안을 슬라이스로 닫았다(#338 SPEC · #339 구현). `!` 함수가 부른 무한 함수(export 포함) 안의 산술도 넘치면
> `-3`이 전파되고, 같은 함수를 무한 본문·호스트가 부르면 wrap한다. 바인딩은 한 집합으로 선언하고, 닿지 않는 모듈의
> 생성 Rust는 글자 그대로 같다. 조용한 오답 60 → 44(626행, 시험 구현과 같다). DP-O7(무한 헬퍼 = 탈출구)은 뒤집혔다.
>
> **▶ 다음 세션 — 여기서 시작한다.** 순서대로:
>
> 1. **넓은 중간값 — 확정(2026-09-27), 구현 중.** `docs/slices/SPEC-wide-intermediates.md`의 DP-W1~W11을 권고안대로
>    확인받았다(**순수 트리만** — 멈춤·앞지름 없이 참값이 드는데 `-3`인 행 59 → 20). §6 작업 1(codegen)이 들어가면 남는 것은
>    작업 2 — 기존 예제의 한 식을 실제 C 호스트로 재고, 626행을 구현본으로 다시 잰다. 시험 구현은 로컬 브랜치
>    `exp/wide2-local`에 있다(푸시 안 함).
...
> 강한 신호는 CI 두 잡이다.
>
> 📌 **본문은 §9-72다**(`docs/history/9-72.md`). 그 앞의 2026-09-26 (5) 항목(체크드 산술 다음 측정)은
> 바이트 그대로 옮겼다 — 지금은 `docs/history/start-block-053.md`다.
>
>
