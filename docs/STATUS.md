...
### 3a. 즉시 재개 (확인/블록 대기)
- **수치 변환(`as`) SPEC (PR #58) — 사용자 확인 대기.** 후보를 나열하는 대신 **실제 업무 규칙을
  오늘의 언어로 써 봤고**, 단가 × 수량(`f64 * i32`)에서 막혀 **곱셈을 `while` 루프로 흉내내야** 했다.
  핵심 결정은 **DP-N2**(`f64 as i32`의 절단·포화·NaN 의미를 언어 규칙으로 못박기)와
  **DP-N4**(`i32 /`는 호스트를 죽이므로 별도 슬라이스).
