...
> (PR #30, 규칙 8상 사용자 승인)에 반영 완료. 이 문서는 이제 **설계 기록**이다 — 닫힌 제안을 다시 열지
> 않는다.
> **근거 수준:** E2 실측 — 오라클이 산출 DLL을 로드해 성공(`status=0`, out 기록)·실패(`status=1`, out 미변경)
> 두 경로 assert. (작성 시점엔 수용 D가 BLOCKED였다 — 이후 C 쪽은 닫혔고 Delphi는 미검증.) 최신 실측은 `docs/STATUS.md`.
...
## 4. Q13 확정 반영 (문서 절차) — **완료**

Q13(status 체계) = **평탄 i32: 0=OK / 양수=모듈 도메인 에러 / 음수=예약(런타임·ABI)** — 사용자 확정 2026-08-28.
반영 완료: `OPEN_QUESTIONS.md`의 Q13 close(**PR #28**), `DECISIONS.md`의 D17 확정 상세(**PR #30**, 규칙 8상 사용자 승인 후 별도 문서 PR). 이 SPEC 자체는 `DECISIONS.md`를 바꾸지 않았다.

## 5. 설계 제안 (DP — **확인됨 2026-08-28**)
...
> **DP-E1 세부(Grok 지적) — 확정됨:** 실패 가능 여부를 본문 `fail` 유무로 **추론**하면, 나중에 `fail`을 추가하는 순간 시그니처가 `T 반환` ↔ `int32 status + out`으로 바뀌어 **ABI 파괴**가 된다. → **명시적 표식 `-> T!` 채택**(사용자 확정 2026-08-28, PR #15로 구현). 표식이 없는 함수는 실패할 수 없고 값을 직접 반환한다.
...
## 7. WBS (이 슬라이스 — **WE1~WE5 완료**, PR #15에 한 번에 반영)
...
> 절차(수행됨): SPEC 확인(2026-08-28) → WE1..WE5 [실패 테스트 → 구현 → 통과 → Grok 검증] → **PR #15 merge**.
