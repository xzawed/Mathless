...
/// **A SPEC the index calls closed says so in its own header, and says it is a record.**
///
/// Why: eleven SPECs the index marked ✅ had headers saying 구현 대기 or 구현 중. A closed SPEC's
/// body is the record of the day it closed and is not edited again, so its header has to say so
/// before a reader takes the body as current. (#261, ⑤)
#[test]
fn a_closed_spec_says_so_in_its_own_header() {
    const RECORD: &str = "**기록 문서다.**";
    ...
    for line in index.lines() {
        ...
        if !file.starts_with("SPEC-") || !file.ends_with(".md") {
            continue;
        }
        let spec = read(&format!("docs/slices/{file}"));
        // Every closed SPEC — shipped (✅) or decided against (⛔) — opens with the record
        // banner: its body is not kept current, and the reader has to know that first.
        if (cells.contains('✅') || cells.contains('⛔'))
            && !spec.lines().take(6).any(|l| l.contains(RECORD))
        {
            offenders.push(format!(
                "{file}: the index calls it closed, but its first lines do not carry the record \
                 banner `{RECORD}` — add it under the title: the body is the record of the day \
                 it closed and is not edited again"
            ));
        }
        if !cells.contains('✅') {
            continue;
