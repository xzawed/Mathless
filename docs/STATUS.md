...
6. **README의 CI 뱃지 — 실측 404.** 저장소가 **private**이라
   `actions/workflows/ci.yml/badge.svg`가 인증 없이 **HTTP 404**를 반환한다(측정). GitHub은 README
   이미지를 익명 프록시로 가져오므로 깨진 이미지로 보일 가능성이 크다.
   - **선택지:** (a) 저장소를 공개하면 저절로 해결, (b) 뱃지를 뺀다, (c) 그대로 둔다(공개 전환 시 부활).
   - 지금은 **그대로 뒀다** — 제거는 되돌리기 번거롭고, 공개 여부가 먼저 정해질 문제라서다.
