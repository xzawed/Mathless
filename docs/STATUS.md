...
> **▶ 다음 세션 — 여기서 시작한다.** 순서대로:
>
> 1. **다음 슬라이스 선택 — 사용자 결정.** 후보의 정본은 `docs/slices/README.md` "다음 슬라이스"다. §7과 plan
>    게이트는 이번 세션에 돌렸다 — Grok `CHOICE`는 **호스트에 나가는 상수 선언**(§9-67.4). 고르면 SPEC →
>    사용자 확인 → 구현이고, SPEC의 첫 항목은 생성 매크로(`IFACE_HASH`·`H`)와의 명명 충돌이다.
