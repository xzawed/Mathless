...
Delphi, C, C++, C# 같은 네이티브 호스트를 위해 만들었습니다.
...
이 모든 과정이 컴파일 타임에 끝납니다. 런타임에 무언가를 해석하지 않고, 바이트코드 VM도 없습니다.
...
**현재 언어.** 타입은 `f64`, `bool`, `i32` 세 가지입니다. 산술(`+`, `-`, `*`, 그리고 `f64`에서만
되는 `/`)과 비교가 있고, 두 수치 타입 사이에는 명시적 `as` 변환이 있습니다. 제어 흐름은 `if`, `while`,
`return`이며 `else`는 아직 없습니다. 지역 변수는 `let`과 `let mut`이고 대입을 지원합니다. 연산자로는
단항 `-`와 `!`, 그리고 `&&`와 `||`가 있습니다. 함수는 실패 가능하게 선언할 수 있습니다. `-> T!`에
`error NAME = N`과 `fail NAME`을 쓰면, 정수 status와 out-param으로 내려갑니다. 내부 `fn`끼리 서로
호출할 수 있지만, 재귀는 컴파일 타임에 거부합니다. 정본 목록은
