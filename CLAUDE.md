...
- **C ABI가 유일한 1급 경계** (`HOST_ABI.md`): 언어별 바인딩은 C ABI 위의 얇은 래퍼다. MVP 마샬링 **목표**는 정수/실수/bool/C 문자열/합의된 레이아웃의 구조체 포인터/1단계 콜백까지다. **오늘 무엇이 구현됐는지는 여기 적지 않는다** — 정본은 `HOST_ABI.md`의 "현재 구현된 경계"다(#260). 변하지 않는 것만 남긴다: 가변 길이 데이터는 Q12(caller-allocates)를 따른다, 경계 밖 인덱스는 `ML_ST_INDEX_OUT_OF_RANGE = -2`다. 모듈은 호스트 메모리를 소유하지 않는다. 예외가 ABI를 가로지르지 않게 한다.
- **보호는 단계적** (`SECURITY.md`): 목표는 "리버싱 불가"가 아니라 분석·변조 비용을 높이는 것. **오늘은 P0까지**(소스 미배포 + 심볼 strip + 최소 export, 실측). **P1·P2는 전부 ⏳ 미구현**이고 확정 결정인 개발/배포 빌드 분리(D13)도 거기(P1) 있다 — 목록은 `SECURITY.md`가 정본이다.

## 반드시 지킬 것

1. `docs/DECISIONS.md`의 **확정 결정 전부**를 뒤집지 않는다 — 범위는 그 문서가 정본이라 여기 번호를 적지 않는다(#143). 뒤집으려면 그 문서에 근거와 트레이드오프를 **먼저** 쓴다.
...
8. **`DECISIONS.md`는 사용자 확인 없이 바꾸지 않는다.** `OPEN_QUESTIONS.md`의 **열린 질문**에 결정안을 제안할 때는 장단점 표를 쓰고 문서 수정 전에 확인을 받는다. **어느 질문이 열려 있는지는 그 문서가 정본이라 여기 번호를 적지 않는다**(#299).
...
- **verify 프롬프트는 좁은 닫힌 질문으로 쓴다** — 2~3개의 판정 가능한 주장 + 파일·줄 범위 + 분량 상한 + 고정 출력형(CONFIRMED/DISCREPANCY, PASS/FAIL) + **REVIEW ONLY**(도구가 파일을 고칠 수 있다). 넓은 질문과 *반례를 찾아라* 같은 열린 질문은 타임아웃하므로, 반례 탐색은 이쪽에서 프로그램으로 돌린다(#46, #207, STATUS §7-1).
- **verify의 지적은 코드로 재확인한 뒤 반영한다** — 옳은 결함을 잡는 만큼 기존 동작을 새 회귀로 오인하기도 해서, 그대로 고치면 잘못된 특수 처리를 낳는다(#46).
...
1. 수용 D의 Delphi 절반은 **로컬 게이트까지**이고 CI로는 여기서 못 닫는다(러너에 Delphi도 대화형 세션도 없다) — 현황은 `docs/STATUS.md`, 경위는 §9-20.
2. 다음 슬라이스는 `docs/STATUS.md` §9(**▶ 여기서 시작한다** 블록)에서 고른다. **닫힌 슬라이스를 다시 열지 않는다** — 닫힌 목록의 정본은 `docs/slices/README.md` 색인이다. **여기에 개수나 이름을 나열하지 않는다**(#133). 세려면 `ls docs/slices/SPEC-*.md | wc -l`.
3. **이후 슬라이스 후보를 여기 나열하지 않는다** — 정본은 `docs/slices/README.md`의 "다음 슬라이스 (SPEC 미작성)" 절이고 가드는 그 절만 지킨다(`no_closed_slice_is_still_listed_as_a_candidate`, #260). **DP-H3(b)는 후보가 아니다** — *하지 않는다*로 닫혔고(`SPEC-symbol-embedded-hash.md`, #141), 재검토 조건 둘이 충족되기 전에는 다시 열지 않는다.
...
호스트를 재컴파일하지 않고 Mathless로 컴파일한 네이티브 모듈을 로드해, 타입이 있는 함수(예: `discount(price, vip)`)를 호출할 수 있으면 1차 성공이다. → **달성했다.** 어느 호스트가 어디까지 게이트되는지는 `docs/STATUS.md`가 정본이라 여기 복제하지 않는다(#285).
