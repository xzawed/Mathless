...
> **▶ 다음 세션 — 여기서 시작한다.** 순서대로:
>
> 1. **다음 슬라이스 — 사용자 결정.** 후보의 정본은 `docs/slices/README.md`의 "다음 슬라이스" 절이다. 가장 가까운 것은
>    **넓은 중간값(WIDE)** 이다 — 참값이 드는데 `-3`인 행을 줄이지만 §9-71.4의 위험을 SPEC이 정해야 하고, 헬퍼 체크 전파
>    뒤로 그 표를 다시 재야 한다(§9-72.7). 시험 구현은 로컬 브랜치 `exp/wide-local`에 있다(푸시 안 함).
