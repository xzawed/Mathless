...
|---------|------|--------------|---------|
| [내부 함수와 호출](SPEC-calls.md) | 📝 **초안 — 사용자 확인 대기** | 지금은 **로직을 나눌 수단이 없다**(모든 함수가 export, 서로 호출 불가). 헬퍼는 export되지 않으므로 D04/D05 프록시가 그대로 유지된다. **재귀는 금지 제안**(§5.1 — `while`과 달리 판정 가능하고 결과가 더 나쁘다) | #56 |
