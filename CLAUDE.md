...
- **C ABI가 유일한 1급 경계** (`HOST_ABI.md`): 언어별 바인딩은 C ABI 위의 얇은 래퍼다. MVP 마샬링 **목표**는 정수/실수/bool/C 문자열/합의된 레이아웃의 구조체 포인터/1단계 콜백까지다. **오늘 구현된 것은 스칼라(f64/bool/i32)·문자열·배열 입력이다** — 문자열은 **파라미터**(`const char*`, 빌림)와 **`-> string!` 반환**(Q12 caller-allocates), 연결 `+`, `i32 as string`까지이며(#89·#92·#108), **배열은 입력만**(빌린 포인터 + 컴파일러가 붙이는 길이, 읽기 전용, 경계 검사 → `ML_ST_INDEX_OUT_OF_RANGE = -2`). **배열 반환·구조체·콜백은 여전히 ⏳ 미구현**이다. 범위의 정본은 `HOST_ABI.md`의 "현재 구현된 경계"다. 모듈은 호스트 메모리를 소유하지 않는다. 예외가 ABI를 가로지르지 않게 한다.
