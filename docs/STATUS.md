...
성격별로 묶어 둔다. **언어 설계 = 1·2** · **공개 위생 = 4·5·9·10** · **범위 = 7·8** ·
**법률 = 3**(급하지 않다고 하셨다). **6은 닫혔다**(공개와 함께 저절로 풀렸다 — 아래).

**번호는 고정한다.** 닫힌 항목도 지우지 않고 ✅로 남긴다 — 다른 문서와 PR이 "§4-6" 같은 번호로
가리키기 때문이다.

다음 세션이 §3a 슬라이스로 바로 간다면 **1번만 먼저 물으면 된다** — 파서를 건드리는 유일한 항목이다.
9번은 코드와 무관하지만 **미루면 손해가 누적되는 유일한 항목**이라 같이 물을 만하다.
...
4. **패닉의 결과를 여섯 곳에서 잘못 적고 있었다 — 정정함(2026-08-31).** ↓
...
**남은 것은 §4-9 하나뿐**(커밋 이메일). 파일이 아니라 git 메타데이터라 위 스캔에 잡히지 않는 자리다.
