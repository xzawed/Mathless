...
/// Refuse a module that carries array-return statements, naming the reason.
///
/// AR1 landed the surface -- `-> [T]!`, `result n`, `result[i] = v` all parse and typecheck.
/// AR3 lands the lowering. In between, a module using them must be refused HERE, for two
/// reasons:
///
/// 1. `emit_stmt` cannot return an error, so its arm for these is `unreachable!` -- and that
///    is only true while this guard stands. Without it the compiler PANICS on valid input.
/// 2. Without it the author hears codegen's all-paths-return safety net instead:
///    "function 'f' may not return on all paths". Measured, and it is the failure shape this
///    repository keeps finding (9-12): a true sentence pointing at the wrong thing. The
///    function is correct; it is the compiler that is half-finished, and the message should
///    say which.
fn refuse_unlowered_array_return(module: &IrModule) -> Result<(), CodegenError> {
    fn carries(body: &[IrStmt]) -> bool {
        body.iter().any(|s| match s {
            IrStmt::ResultLen(_) | IrStmt::ResultSet { .. } => true,
            IrStmt::If { body, .. } | IrStmt::While { body, .. } => carries(body),
            // Listed rather than wildcarded, by the crate's deny: a new statement form has to
            // answer whether it can carry an array return rather than defaulting to "no".
            IrStmt::Return(_)
            | IrStmt::Fail(_)
            | IrStmt::Let { .. }
            | IrStmt::Assign { .. }
            | IrStmt::AssignOut { .. }
            | IrStmt::TryCall { .. } => false,
        })
    }
    for f in &module.functions {
        if carries(&f.body) {
            return Err(CodegenError::new(format!(
                "function '{}' returns an array. The frontend accepts this (AR1), but the \
                 backend does not lower it yet (AR3 of SPEC-array-return): there is no \
                 buffer, capacity or needed-length parameter emitted for it. Nothing is \
...
pub fn emit(module: &IrModule) -> Result<String, CodegenError> {
    refuse_unlowered_array_return(module)?;
...
fn emit_stmt(s: &IrStmt, indent: usize, abi: RetAbi, out: &mut String) {
    ...
    match s {
        ...
        IrStmt::ResultLen(_) | IrStmt::ResultSet { .. } => unreachable!(
            "array return is not lowered yet (AR3); `emit` refuses these modules first"
        ),
