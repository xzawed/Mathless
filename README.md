...
Measured on Windows (`cargo test --workspace` = **123 green**; CI runs `windows-latest` — the authority, where acceptance A/B/C/D actually execute — plus `ubuntu-latest` for the frontend; toolchain pinned):
...
- **Language today** — `f64` / `bool` / `i32`, `if` / `return`, **fallible functions** (`-> T!` = integer
  status + out-parameter), **locals** — `let` / `let mut` + assignment, **`while` loops**.
