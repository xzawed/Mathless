...
| [내부 함수와 호출](SPEC-calls.md) | ✅ 구현 완료 | 그전에는 **로직을 나눌 수단이 없었다**. 헬퍼는 export되지 않음을 C 호스트의 `GetProcAddress`로 측정. **재귀는 금지**(§5.1 — 판정 가능하고 결과가 프로세스 종료) | #56 / #57 |
| [수치 변환 `as`](SPEC-numeric-conversion.md) | ✅ 구현 완료 | 그전에는 **단가 × 수량이 안 돼 곱셈을 루프로 흉내내야 했다**. `f64 as i32`의 절단·포화·NaN을 언어 규칙으로 못박고 **모두 측정** | #58 / #59 |
