...
> **▶ 다음 세션 — 여기서 시작한다.** 순서대로:
>
> 1. **헬퍼 체크 전파 SPEC 확인 — 사용자 결정.** 방향(§9-71.6의 F2)은 확인받았다(2026-09-26). 초안은
>    `docs/slices/SPEC-helper-check-propagation.md`이고 §4의 DP-P2~P11이 확인을 기다린다. 확인되면 §6의 두 PR로
>    구현한다. 시험 구현은 로컬 브랜치 `exp/wide-local`에 있다(푸시 안 함).
