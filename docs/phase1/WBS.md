...
>
> **STEP1(Gate-D prep) ✅**: `mlc build <f.mls> -o <dir>` CLI가 `.dll`+`.h`+`.pas` 3종을 디스크로 산출한다(라이브러리 `emit::emit_artifacts`, bin은 argv만 — **당시 3종이고, `.lib`가 2026-09-03에 합류해 오늘은 4종이다**). 실측 E2(STEP1 당시): `cargo test` 30 그린(현재 값은 `docs/STATUS.md`), 오라클이 **산출 dll**을 로드해 `mlx_discount(100,true)=90`/`abi_version=1`·export 2개 통과, 실 CLI 실행이 `discount.dll(9,728 B)`+`.h`+`.pas` 생성. (당시) `.h`/`.pas`의 실제 로드는 미검증이었다 — **`.h`는 이후 W12에서 해소**, `.pas`도 이후 닫혔다(아래 W7 행 — 이 줄만 오래 DRAFT 상태로 적고 있었다).
>
...
| **W17** | **수치 변환 `as`** — 그전엔 단가 × 수량이 안 됐다 | [`SPEC-numeric-conversion.md`](../slices/SPEC-numeric-conversion.md) | `qty as f64`로 곱셈 가능(이전엔 루프로 흉내); `true as i32` 거부; 암시적 혼합은 여전히 거부; `-x as f64` = `(-x) as f64`(#68에서 Rust 결합으로 뒤집힘 — 당시에는 `-(x as f64)`였다). **경계 4종을 실제 모듈에서 측정**: 절단(3.9→3, -3.9→-3)·포화(±inf→MAX/MIN)·NaN→0·`i32::MAX as f64` 정확. C 호스트 통과 | #58(SPEC) / #59 |
| **W16** | **내부 함수와 호출** — 그전엔 로직을 나눌 수단이 없었다 | [`SPEC-calls.md`](../slices/SPEC-calls.md) | 비-export `fn` + 호출식; 2-패스 이름 해석; 인자 개수·타입 검사; **직접·상호 재귀 거부**(경로 표시); 헤더/유닛에서 내부 함수 제외. **핵심 측정: 헬퍼가 있는 모듈의 export가 정확히 2개이고, 실제 C 호스트의 `GetProcAddress("vip_rate")`가 NULL** | #56(SPEC) / #57 |
| **W15** | **논리 연산자 `&&` · `\|\|`** — 우회 수단이 없던 유일한 부류 | [`SPEC-logical-ops.md`](../slices/SPEC-logical-ops.md) | `while i < n && i < cap` 컴파일(이전 렉스 에러); 비-bool 피연산자 거부; `a && b \|\| c` = `((a&&b)\|\|c)`; `a & b`는 "`&&`를 의도했나?"로 진단; 오라클·**실제 C 호스트** 통과; export 불변. **단락 평가는 명세하되 미측정**(관측 수단 없음) | #54(SPEC) / #55 |
...
> 두 번째 호스트를 먼저 착수하지 말 것"을 담고 있지 않다 — 그 순서는 STATUS **§3b**가 후보
> 목록으로 다룬다.
