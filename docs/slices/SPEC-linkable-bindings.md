...
**상태: 확정 · 구현 완료 (2026-09-03).** 사용자 확인 후 TDD로 구현했다 —
#124(`.lib` 4번째 산출물 + 문서 가드) · #125(헤더 18개 C++ 게이트) · #126(링크 호스트 + 드리프트 거부).
**결정 근거:** `STATUS.md` §9 D1 — 사용자 결정(2026-09-03): 다음 슬라이스는 배열도 struct도 아닌
**ABI 정합**이다.

> **이 문서는 작성 시점의 설계 기록이다**(`slices/README.md`의 규약). 아래 §6의 두 열린 질문은
> 구현하면서 권고안대로 닫혔다: 파일 이름은 **`<module>.lib`**, `LICENSE-OUTPUT-EXCEPTION`에
> **한 줄 명시**. §5의 "18개 헤더를 한 TU에서"는 **구현과 다르다** — 자립성까지 보려고
> **헤더당 한 TU**로 갔고, 전부를 한 번의 `cl` 호출에 넘긴다.
